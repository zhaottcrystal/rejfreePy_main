...
#The true stationary distribution is
#[ 0.02648991  0.03798742  0.00474611  0.20722239  0.00668647  0.01731848
#  0.0664469   0.0115679   0.01395126  0.01619195  0.06975407  0.39771265
#  0.04189059  0.01313915  0.06889476]
...
for i in range(nMCMCIters):
    ...
    for j in range(nPairSeq):
        suffStat = endPointSamplerSummarizeStatisticsOneBt(True, prng, initialRateMatrix, firstLastStatesArrayAll[j],
                                                           1.0)
    ...
    sample = prng.uniform(0, 1, len(avgWeights))
...
download_dir = "timeElapsedHMC3States1000.csv" #where you want the file to be downloaded to
...
np.savetxt('stationaryDistributionHMC3States1000.csv', stationarySamples, fmt='%.3f', delimiter=',')
np.savetxt('stationaryWeightHMC3States1000.csv', stationaryWeightsSamples, fmt='%.3f', delimiter=',')
np.savetxt('exchangeableParametersHMC3States1000.csv', exchangeableSamples, fmt='%.3f', delimiter=',')
np.savetxt('binaryWeightsHMC3States1000.csv', binaryWeightsSamples, fmt='%.3f', delimiter=',')
np.save('3dsaveHMC3States1000.npy', rateMatrixSamples)
